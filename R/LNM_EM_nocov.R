...
LNM.EM.nocov <- function(W, base, EMiter = 10, EMburn = 5, MCiter = 1000, MCburn = 500, stepsize = 0.01, p = 0.05, 
    ...
    # Should be (MCiter x Q x N) (1000 x 75 x 119) Dont forget, first column is acceptance (ie want 74 x 119 for data)
    for (em in 1:EMiter) {
        cat("EM iteration:", em, "\n")
        start <- proc.time()
        ...
        # should call 119 apply functions, each time, get 75 means. each of iteration values for OTU.  ORIGINAL for(i in
        # 1:119) { Y.new[i,] <- apply(MCarray[(MCburn+1):MCiter,,i],2,mean) } ALTERNATIVE, no for loop if large N, but
        # transpose: seems faster by system.time
        ...
        # sigSums <- matrix(0,Q-1,Q-1) # Get all sample matrices for(i in (MCburn+1):MCiter) { # first part is Y sample
        # Eps.samp <- t(MCarray[i,2:Q,1:N]) - eY sigSums <- sigSums + crossprod(Eps.samp) } CAN EASILY MAKE ABOVE APPLY:
        # next just take mean
    ...
    ## Next: take average of EM samples past burn. Included init, so have EMiter+1 total Note, below doesn't have any
    ## inital input
