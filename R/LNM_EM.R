...
#'
#' @export
LNM.EM <- function(W, base, EMiter = 10, EMburn = 5, MCiter = 1000, MCburn = 500, stepsize = 0.01, p = 0.05, 
    poorman = FALSE) {
    ...
    for (em in 1:EMiter) {
        ...
        # Y sample Eps.samp <- t(MCarray[i,2:Q,1:N]) - eY sigSums <- sigSums + crossprod(Eps.samp) } CAN
